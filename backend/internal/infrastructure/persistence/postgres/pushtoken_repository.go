...
func (r *PushTokenRepository) Save(ctx context.Context, t *pushtoken.Token) error {
	// endpoint is NULL for FCM/APNs rows; for web push rows it carries the
	// JSON-encoded {p256dh, auth} blob alongside the URL stored in `token`.
	var endpoint any
	if t.Endpoint != "" {
		endpoint = t.Endpoint
	}
	_, err := r.pool.Exec(ctx, `
		INSERT INTO push_tokens (id, user_id, platform, token, endpoint, last_seen, created_at)
		VALUES ($1,$2,$3,$4,$5,$6,$7)
		ON CONFLICT (platform, token) DO UPDATE
			SET user_id = EXCLUDED.user_id,
			    endpoint = EXCLUDED.endpoint,
			    last_seen = EXCLUDED.last_seen`,
		t.ID, t.UserID, string(t.Platform), t.Token, endpoint, t.LastSeen, t.CreatedAt,
	)
	return mapError(err)
}
...
func (r *PushTokenRepository) ListByUser(ctx context.Context, userID uuid.UUID) ([]*pushtoken.Token, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT id, user_id, platform, token, endpoint, last_seen, created_at
		FROM push_tokens
		WHERE user_id = $1
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.UserID, &platform, &t.Token, &endpoint, &t.LastSeen, &t.CreatedAt); err != nil {
			return nil, mapError(err)
		}
